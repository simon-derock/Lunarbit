...
- Active phase: Product hardening and cloud-deployment preparation. The Aura-backed GraphRAG runtime, secure SSE Ask flow, browser proxy, durable SQLite session/checkpoint store, Cortex favicon, citation identifiers, canonical merchant identity resolution, evidence-backed spend totals, and complete economic graph ingestion are implemented and pushed; remaining gates are human-reviewed language quality, full HITL pause/resume, durable production hosting, live E2E coverage, and final security/privacy review.
- Current branch: `main`, pushed to `origin/main`
- Repository: `https://github.com/simon-derock/Lunarbit.git`
- Last verified implementation commit: `2254270` (`feat(session): harden durable conversation lifecycle`), pushed to `main`.
- Recent commits: `2254270` durable session TTL/capacity hardening, `354137a` missing-slot abstention contract repair, `e9e6058` temporal spending regime shifts, `408d361` merchant concentration metrics.
- Last passing checks: 347 Python tests, Ruff format/lint, strict MyPy across 45 modules, repository hygiene, frontend Vitest (12 tests), frontend TypeScript/Vite production build, live Aura snapshot and authenticated browser-proxy SSE verification (346 nodes/99 edges), standalone public-only Aura release audit, browser-origin SSE proxy smoke test, and production API image build/non-root inspection.
