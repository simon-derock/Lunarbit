...
- Last verified implementation commit: `f773b55` (`test(release): strengthen stream and golden coverage`), pushed to `main`.
- Recent commits: `f773b55` stream/golden regression coverage, `3cbe26e` public readiness release gate, `a7d4535` durable HITL session history, `e6a3819` evaluation contract v1.1.
- Last passing checks: 316 collected Python tests (240 non-HTTP contracts pass in the current sandbox), Ruff format/lint, strict MyPy across 45 modules, repository hygiene, frontend Vitest (10 tests), frontend TypeScript/Vite production build, live Aura snapshot and chat verification from the latest completed runtime pass, browser-origin SSE proxy smoke test, and production API image build/non-root inspection.
