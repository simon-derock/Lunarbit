...
- Last verified implementation commit: `549d439` (`fix(security): harden API origin and browser headers`), pushed to `main`.
- Recent commits: `549d439` API origin/header hardening, `3396a86` frontend retry/incomplete-stream coverage, `a0fd009` responsive browser smoke verifier, `94757b5` SSE exception redaction.
- Last passing checks: 317 collected Python tests (240 non-HTTP contracts pass in the current sandbox), Ruff format/lint, strict MyPy across 45 modules, repository hygiene, frontend Vitest (12 tests), frontend TypeScript/Vite production build, live Aura snapshot and chat verification from the latest completed runtime pass, browser-origin SSE proxy smoke test, and production API image build/non-root inspection.
...
- In progress: Public privacy review and cloud deployment preparation. The current public UI consumes only the reviewed FastAPI contract. HITL now exposes explicit clarification, identity-ambiguity, and financial-conflict review metadata; graph writes remain disabled.
