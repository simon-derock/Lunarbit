...
- Active phase: Product hardening and cloud-deployment preparation. The Aura-backed GraphRAG runtime, secure SSE Ask flow, browser proxy, durable SQLite session/checkpoint store, authenticated HITL review transitions and approved-answer resumption, Cortex favicon, citation identifiers, canonical merchant identity resolution, evidence-backed spend totals, and complete economic graph ingestion are implemented and pushed; remaining gates are human-reviewed language quality, durable production hosting, live E2E coverage, and final security/privacy review.
- Current branch: `main`, pushed to `origin/main`
- Repository: `https://github.com/simon-derock/Lunarbit.git`
- Last verified implementation commit: `c5837a6` (`ci(aura): exercise authenticated HITL resume`), pushed to `main`.
- Recent commits: `c5837a6` live Aura HITL resume smoke, `d7b9996` HITL answer resumption, `3863243` review-state documentation, `45d94d0` authenticated HITL transitions.
- Last passing checks: 349 Python tests, Ruff format/lint, strict MyPy across 45 modules, repository hygiene, frontend Vitest (14 tests), frontend TypeScript/Vite production build, live Aura snapshot and authenticated browser-proxy SSE verification (346 nodes/99 edges), standalone public-only Aura release audit, browser-origin SSE proxy smoke test, and production API image build/non-root inspection.
...
3. **Conversation durability:** use the authenticated SQLite-backed session/checkpoint store when `LUNARBIT_SESSION_DB` is configured; history listing, TTL, capacity limits, review-state persistence, and restart recovery are covered. Keep the process-local store only for tests/development.
4. **HITL:** pause for sensitive scope expansion, identity ambiguity, or any mutation proposal; authenticated operators can transition pending turns exactly once through `/v1/private/chat/{session_id}/review` and resume the original bounded read-only answer through `/v1/private/chat/{session_id}/review/resume`; graph writes remain disabled.
...
9. **Release automation:** CI gates for Python/frontend lint, type checks, TDD, integration, live Aura SSE plus HITL resume smoke tests, privacy-leakage checks, build artifacts, migration checks, and deployment smoke tests. Browser-level Playwright coverage remains a deployment-target gate.
