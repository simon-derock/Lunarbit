...
def _templates_for(question: str, intent: QueryIntent) -> tuple[QueryTemplate, ...] | None:
    ...
    if any(token in normalized for token in ("discount", "coupon", "promotion")) and any(
        token in normalized for token in ("fee", "delivery", "charge")
    ):
        return (QueryTemplate.FEE_DISCOUNT_ANALYSIS,)
    if any(
        token in normalized for token in ("caused", "decomposition", "break down", "breakdown")
    ) and any(token in normalized for token in ("spending", "spend", "cost")):
        return (QueryTemplate.SPENDING_CHANGE_DECOMPOSITION,)
    if any(
        token in normalized for token in ("would have saved", "waived", "counterfactual")
    ) and any(token in normalized for token in ("delivery fee", "delivery charge", "fees")):
        return (QueryTemplate.DELIVERY_FEE_COUNTERFACTUAL,)
    if any(
        token in normalized
        for token in (
            "increased in price",
    ...
    ) and any(token in normalized for token in ("dish", "dishes", "item", "items", "price")):
        return (QueryTemplate.ITEM_PRICE_CHANGE_RANKING,)
    if any(
        token in normalized
        for token in (
            "food price index",
        ...
        return (QueryTemplate.PERSONAL_FOOD_PRICE_INDEX,)
